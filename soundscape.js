...
(() => {
  ...
  function start() {
    if (started) { muted = false; master.gain.setTargetAtTime(.72, ctx.currentTime, .08); ctx.resume(); window.__replicSound.enabled=true; return; }
    ...
    window.__replicSound = { ctx, master, rain, wind, sea, traffic, siren, sirenGain, rotor, rotorGain, enabled:true };
  ...
  button.addEventListener('click', e => {
    ...
    else { muted = true; if(window.__replicSound) window.__replicSound.enabled=false; master.gain.setTargetAtTime(0, ctx.currentTime, .1); button.textContent = '🔇'; button.setAttribute('aria-label', 'Activar sonido'); }
  }, true);
  ...
  function thunder(distance) {
    ...
    osc.type = 'sawtooth'; osc.frequency.setValueAtTime(78 + Math.random() * 20, now); osc.frequency.exponentialRampToValueAtTime(25, now + 1.8);
    filter.type = 'lowpass'; filter.frequency.value = 430;
    const volume = Math.max(.015, .18 * Math.exp(-distance / 65));
    gain.gain.setValueAtTime(.0001, now); gain.gain.exponentialRampToValueAtTime(volume, now + .07); gain.gain.exponentialRampToValueAtTime(.0001, now + 2.2);
  ...
  function tick(t) {
    ...
    const storm = distanceGain(tornadoD, 4, 105), sea = distanceGain(seaD, 8, 140), city = distanceGain(cityD, 6, 120), airport = distanceGain(airportD, 8, 65);
    const now = ctx.currentTime;
    s.wind.gain.setTargetAtTime(s.wind.base * (.18 + storm * 1.65), now, .12); s.wind.filter.frequency.setTargetAtTime(280 + storm * 900, now, .15);
    s.rain.gain.setTargetAtTime(s.rain.base * (.12 + storm * 1.25), now, .18); s.sea.gain.setTargetAtTime(s.sea.base * sea, now, .22);
    s.traffic.gain.setTargetAtTime(s.traffic.base * city, now, .35); s.rotorGain.gain.setTargetAtTime(.014 * airport, now, .22); s.rotor.frequency.setTargetAtTime(48 + airport * 18, now, .2);
    const pulse=(t%22)/22, active=pulse>.68?Math.min(1,(pulse-.68)/.16)*Math.min(1,(1-pulse)/.16):0;
    s.sirenGain.gain.setTargetAtTime(.014*city*active,now,.08); s.siren.frequency.setTargetAtTime(620+Math.sin(t*4.8)*330,now,.08);
