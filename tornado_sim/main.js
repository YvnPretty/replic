...
(() => {
  ...
  function enableSound() {
    if (!audio.context) {
      ...
      rumbleSource.connect(subFilter).connect(subGain).connect(ctx.destination);
      rumbleSource.start();

      audio.subRumbleFilter = subFilter;
      audio.subRumbleGain = subGain;
      ...
      // 4. Tsunami / Ocean Water Surge Roar
      const surgeSource = ctx.createBufferSource();
      surgeSource.buffer = noiseBuffer;
      surgeSource.loop = true;
      const surgeFilter = ctx.createBiquadFilter();
      surgeFilter.type = 'lowpass';
      surgeFilter.frequency.value = 250;
      const tsunamiGain = ctx.createGain();
      tsunamiGain.gain.value = 0;
      surgeSource.connect(surgeFilter).connect(tsunamiGain).connect(ctx.destination);
      surgeSource.start();

      audio.tsunamiGain = tsunamiGain;

      // 5. Earthquake Sub-Oscillator
      const quakeOsc = ctx.createOscillator();
      quakeOsc.type = 'sine';
      quakeOsc.frequency.value = 32;
      const quakeGain = ctx.createGain();
      quakeGain.gain.value = 0;
      quakeOsc.connect(quakeGain).connect(ctx.destination);
      quakeOsc.start();

      audio.quakeGain = quakeGain;
    }
  ...
  function thunderSound() {
    ...
    // Direct Lightning transient strike snap
    const snapOsc = ctx.createOscillator();
    const snapGain = ctx.createGain();
    snapOsc.type = 'sawtooth';
    snapOsc.frequency.setValueAtTime(140, now);
    snapOsc.frequency.exponentialRampToValueAtTime(30, now + 1.2);
    snapGain.gain.setValueAtTime(0.001, now);
    snapGain.gain.exponentialRampToValueAtTime(0.35, now + 0.03);
    snapGain.gain.exponentialRampToValueAtTime(0.001, now + 1.4);
    snapOsc.connect(snapGain).connect(ctx.destination);
    snapOsc.start();
    snapOsc.stop(now + 1.45);

    // Multi-echo rolling thunder sub-rumble
    for (let echo = 0; echo < 3; echo++) {
      const delayTime = now + 0.1 + echo * 0.35;
      const subOsc = ctx.createOscillator();
      const subGain = ctx.createGain();
      subOsc.type = 'sine';
      subOsc.frequency.setValueAtTime(55 - echo * 8, delayTime);
      subOsc.frequency.exponentialRampToValueAtTime(22, delayTime + 1.6);
      const vol = 0.28 / (1 + echo * 0.6);
      subGain.gain.setValueAtTime(0.001, delayTime);
      subGain.gain.linearRampToValueAtTime(vol, delayTime + 0.08);
      subGain.gain.exponentialRampToValueAtTime(0.001, delayTime + 1.8);
      subOsc.connect(subGain).connect(ctx.destination);
      subOsc.start(delayTime);
      subOsc.stop(delayTime + 1.85);
    }
  }
  ...
  function playRubbleSound() {
    ...
    noise.buffer = createNoiseBuffer(ctx, 1);
    const filter = ctx.createBiquadFilter();
    filter.type = 'bandpass';
    filter.frequency.value = 180 + Math.random() * 200;
  ...
  let dayNightMode = 'auto';
  const dayNightButton = document.getElementById('daynight-toggle');
  if (dayNightButton) {
    dayNightButton.addEventListener('click', () => {
      if (dayNightMode === 'auto') {
        dayNightMode = 'day';
        dayNightButton.textContent = '☀️ Modo: Día Fijo';
      } else if (dayNightMode === 'day') {
        dayNightMode = 'night';
        dayNightButton.textContent = '🌙 Modo: Noche Fija';
      } else {
        dayNightMode = 'auto';
        dayNightButton.textContent = '☀️/🌙 Modo Día/Noche: Auto';
      }
    });
  }

  const targetTopColor = new THREE.Color();
  const targetHorizColor = new THREE.Color();
  const targetSunColor = new THREE.Color();
  const targetHemiSkyColor = new THREE.Color();
  const targetHemiGroundColor = new THREE.Color();
  ...
  scene.add(sky);
  const hemiLight = new THREE.HemisphereLight(0xa059c2, 0x140c21, 1.8);
  scene.add(hemiLight);
  ...
  const kites = [];
  const drones = [];
  let lighthouse, beamCone, lighthouseSpot;
  const buoys = [];
  const dolphins = [];
  ...
  for (let x = -130; x <= 130; x += 5) {
    for (let z = -140; z <= 130; z += 5) {
      ...
      if (x >= -32 && x <= -8 && z >= -82 && z <= -58) continue; // Park area
  ...
  function makeCar(x, z, color, axis, isPolice = false) {
    ...
    // Glowing headlights & red taillights
    const headL = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.08, 0.04), new THREE.MeshBasicMaterial({ color: 0xfffae6 }));
    headL.position.set(-0.28, 0.28, 0.83);
    const headR = headL.clone();
    headR.position.set(0.28, 0.28, 0.83);
    const tailL = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.08, 0.04), new THREE.MeshBasicMaterial({ color: 0xff0022 }));
    tailL.position.set(-0.28, 0.28, -0.83);
    const tailR = tailL.clone();
    tailR.position.set(0.28, 0.28, -0.83);
    car.add(headL, headR, tailL, tailR);

    // Light beam projected on wet pavement
    const headlightBeam = new THREE.Mesh(
      new THREE.PlaneGeometry(1.2, 4.0),
      new THREE.MeshBasicMaterial({ color: 0xfffae6, transparent: true, opacity: 0.18, blending: THREE.AdditiveBlending })
    );
    headlightBeam.rotation.x = -Math.PI / 2;
    headlightBeam.position.set(0, 0.025, 2.8);
  ...
  // Sandy Beach Setup
  const beach = new THREE.Mesh(new THREE.PlaneGeometry(6, 275), new THREE.MeshStandardMaterial({ color: 0xddc48e, roughness: 0.9 }));
  beach.rotation.x = -Math.PI / 2;
  beach.position.set(-29, 0.03, 0);
  world.add(beach);
  
  // Beach Umbrellas and Towels
  for (let z = -120; z <= 120; z += 35) {
    const towel = new THREE.Mesh(new THREE.PlaneGeometry(0.8, 1.4), new THREE.MeshStandardMaterial({ color: [0xff0055, 0x00ffcc, 0xffcc00][Math.abs(z) % 3] }));
    towel.rotation.x = -Math.PI / 2;
    towel.position.set(-28.5 + Math.sin(z) * 0.5, 0.04, z);
    world.add(towel);
    
    const umbrella = new THREE.Group();
    const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 1.4), new THREE.MeshStandardMaterial({ color: 0xdddddd }));
    pole.position.y = 0.7;
    const top = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.9, 0.35, 10, 1, false), new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.6 }));
    top.position.y = 1.35;
    umbrella.add(pole, top);
    umbrella.position.set(-29.8, 0, z + 1.2);
    world.add(umbrella);
  }
  
  // Wooden Pier
  const pier = new THREE.Group();
  const plank = new THREE.Mesh(new THREE.BoxGeometry(14, 0.15, 2.5), new THREE.MeshStandardMaterial({ color: 0x5c4033, roughness: 0.9 }));
  plank.position.set(-36, 0.45, 0);
  pier.add(plank);
  for (let px = -41; px <= -31; px += 3.5) {
    const postL = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 2.2), new THREE.MeshStandardMaterial({ color: 0x2e1a0c }));
    postL.position.set(px, -0.6, -1.1);
    const postR = postL.clone();
    postR.position.set(px, -0.6, 1.1);
    pier.add(postL, postR);
  }
  world.add(pier);

  // City Park at x = -20, z = -70 Setup
  const park = new THREE.Group();
  const lawn = new THREE.Mesh(new THREE.PlaneGeometry(24, 24), new THREE.MeshStandardMaterial({ color: 0x3b7a57, roughness: 0.95 }));
  ...
  const basin = new THREE.Mesh(new THREE.RingGeometry(2.2, 2.5, 32), new THREE.MeshStandardMaterial({ color: 0x778899 }));
  basin.rotation.x = -Math.PI / 2;
  basin.position.y = 0.1;
  const pool = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 2.2, 0.1, 16), new THREE.MeshStandardMaterial({ color: 0x00ffff, roughness: 0.1, metalness: 0.9 }));
  pool.position.y = 0.05;
  const jet = new THREE.Mesh(new THREE.SphereGeometry(0.35, 8, 8), new THREE.MeshBasicMaterial({ color: 0xe0ffff }));
  jet.position.y = 0.35;
  ...
  for (let angle = 0; angle < Math.PI * 2; angle += Math.PI / 2) {
    const bench = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.3, 0.6), new THREE.MeshStandardMaterial({ color: 0x5c4033 }));
    bench.position.set(Math.sin(angle) * 4.5, 0.15, Math.cos(angle) * 4.5);
    bench.rotation.y = angle;
    park.add(bench);
  }
  park.position.set(-20, 0, -70);
  world.add(park);
  for (let angle = 0; angle < Math.PI * 2; angle += Math.PI / 4) {
    makeAnimeTree(-20 + Math.sin(angle) * 9, -70 + Math.cos(angle) * 9);
  }

  // Birds Setup
  for (let i = 0; i < 6; i++) {
    const bird = new THREE.Group();
    const wingL = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.02, 0.38), new THREE.MeshStandardMaterial({ color: 0xffffff }));
    wingL.position.x = -0.19;
    const wingR = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.02, 0.38), new THREE.MeshStandardMaterial({ color: 0xffffff }));
    wingR.position.x = 0.19;
    bird.add(wingL, wingR);
    bird.position.set(-20 + (Math.random() - 0.5) * 8, 12 + Math.random() * 5, -70 + (Math.random() - 0.5) * 8);
    world.add(bird);
    birds.push({
      group: bird,
      wingL,
  ...
  // Kites Setup
  for (let i = 0; i < 3; i++) {
    const kiteGrp = new THREE.Group();
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 0.02), new THREE.MeshBasicMaterial({ color: [0xff0055, 0x00ffcc, 0xffcc00][i] }));
    ...
    kiteGrp.add(tailLine);
    kiteGrp.position.set(-29.8, 8 + i * 2.5, -40 + i * 40);
    world.add(kiteGrp);
    kites.push({
      group: kiteGrp,
      homeY: 8 + i * 2.5,
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const frame = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.04, 0.8), new THREE.MeshStandardMaterial({ color: 0x333333 }));
    const rLight = new THREE.Mesh(new THREE.SphereGeometry(0.06), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
    rLight.position.set(-0.4, 0, -0.4);
    const gLight = new THREE.Mesh(new THREE.SphereGeometry(0.06), new THREE.MeshBasicMaterial({ color: 0x00ff00 }));
    gLight.position.set(0.4, 0, 0.4);
    drone.add(frame, rLight, gLight);
    drone.position.set(-15 + i * 15, 12 + i * 2, 5 + i * 15);
    world.add(drone);
    drones.push({
      group: drone,
      home: drone.position.clone(),
      rLight,
      gLight,
      phase: Math.random() * 10
    });
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.8, 1.0, 3.5, 8), new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.6 }));
  base.position.y = 1.75;
  const band = new THREE.Mesh(new THREE.CylinderGeometry(0.82, 0.88, 1.0, 8), new THREE.MeshStandardMaterial({ color: 0xff0033 }));
  band.position.y = 1.75;
  lighthouse.add(base, band);
  const cabin = new THREE.Mesh(new THREE.CylinderGeometry(0.65, 0.65, 0.8, 8), new THREE.MeshStandardMaterial({ color: 0x222222 }));
  cabin.position.y = 3.9;
  const lighthouseGlass = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.55, 0.6, 8), new THREE.MeshBasicMaterial({ color: 0xffffaa, transparent: true, opacity: 0.8 }));
  lighthouseGlass.position.y = 3.9;
  lighthouse.add(cabin, lighthouseGlass);
  lighthouseSpot = new THREE.SpotLight(0xffffaa, 5, 30, Math.PI / 8, 0.5, 1);
  lighthouseSpot.position.set(0, 3.9, 0);
  lighthouseSpot.target.position.set(-10, 2, 0);
  lighthouse.add(lighthouseSpot);
  world.add(lighthouseSpot.target);
  const beamGeom = new THREE.CylinderGeometry(0.1, 4.5, 20, 16, 1, true);
  beamGeom.translate(0, -10, 0);
  beamCone = new THREE.Mesh(beamGeom, new THREE.MeshBasicMaterial({ color: 0xffff99, transparent: true, opacity: 0.22, side: THREE.DoubleSide, depthWrite: false }));
  beamCone.position.set(0, 3.9, 0);
  beamCone.rotation.x = Math.PI / 2;
  lighthouse.add(beamCone);
  lighthouse.position.set(-43, 0.45, 0);
  world.add(lighthouse);
  buildings.push({
    group: lighthouse,
    home: lighthouse.position.clone(),
    width: 2,
    height: 6,
    cracks: null,
    crackMaterial: null,
  ...
  // Floating Buoys Setup
  const buoyCoords = [
    { x: -35, z: -20, color: 0xff0000 },
    { x: -42, z: 30, color: 0x00ff00 },
    { x: -30, z: -90, color: 0xff0000 }
  ];
  buoyCoords.forEach(pos => {
    const buoy = new THREE.Group();
    const float = new THREE.Mesh(new THREE.SphereGeometry(0.32, 8, 8), new THREE.MeshStandardMaterial({ color: 0xff3300, roughness: 0.4 }));
    float.position.y = 0.2;
    const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.8), new THREE.MeshStandardMaterial({ color: 0x111111 }));
    mast.position.y = 0.6;
    const light = new THREE.Mesh(new THREE.SphereGeometry(0.07), new THREE.MeshBasicMaterial({ color: pos.color }));
    light.position.y = 1.05;
    buoy.add(float, mast, light);
    buoy.position.set(pos.x, 0, pos.z);
    world.add(buoy);
    buoys.push({ group: buoy, homeY: 0, light, color: pos.color, phase: Math.random() * 10 });
  });

  // Small Rocky Island Setup
  const island = new THREE.Group();
  const mound = new THREE.Mesh(new THREE.CylinderGeometry(2.5, 3.5, 0.8, 10), new THREE.MeshStandardMaterial({ color: 0xddc48e, roughness: 0.9 }));
  mound.position.y = 0.2;
  island.add(mound);
  const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.1, 1.8), new THREE.MeshStandardMaterial({ color: 0x5c4033, roughness: 0.9 }));
  trunk.position.set(0, 1.0, 0);
  trunk.rotation.z = 0.15;
  const leaves = new THREE.Group();
  for (let i = 0; i < 5; i++) {
    const leaf = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.02, 1.2), new THREE.MeshStandardMaterial({ color: 0x2e8b57 }));
    leaf.position.set(0, 1.9, 0.5);
    leaf.rotation.x = 0.3;
    leaf.rotation.y = (i * Math.PI * 2) / 5;
    leaves.add(leaf);
  }
  island.add(trunk, leaves);
  island.position.set(-45, 0, -40);
  world.add(island);
  buildings.push({
    group: island,
    home: island.position.clone(),
    width: 6,
    height: 3,
    cracks: null,
    crackMaterial: null,
  ...
  // Jumping Dolphins Setup
  for (let i = 0; i < 3; i++) {
    const dolphinMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 1.2, 8), new THREE.MeshStandardMaterial({ color: 0x5f9ea0, roughness: 0.3 }));
    dolphinMesh.rotation.x = Math.PI / 2;
    const dolphinGroup = new THREE.Group();
    dolphinGroup.add(dolphinMesh);
    world.add(dolphinGroup);
    dolphins.push({
      mesh: dolphinGroup,
      progress: Math.random() * Math.PI,
  ...
  // ==========================================
  // 1. ELEVATED MONORAIL / TRAIN SYSTEM
  // ==========================================
  const monorailGroup = new THREE.Group();
  const railX = 22;
  const railY = 6.8;
  const trackBeam = new THREE.Mesh(
    new THREE.BoxGeometry(0.8, 0.45, 270),
    new THREE.MeshStandardMaterial({ color: 0x4a525d, roughness: 0.5, metalness: 0.5 })
  );
  trackBeam.position.set(railX, railY, 0);
  monorailGroup.add(trackBeam);

  // Pillars along monorail
  for (let pz = -135; pz <= 135; pz += 22) {
    const pillar = new THREE.Mesh(
      new THREE.CylinderGeometry(0.4, 0.5, railY, 8),
      new THREE.MeshStandardMaterial({ color: 0x323842, roughness: 0.7 })
    );
    pillar.position.set(railX, railY / 2, pz);
    monorailGroup.add(pillar);
  }

  // Monorail Train (3 cars)
  const monorailTrain = new THREE.Group();
  const trainCars = [];
  for (let c = 0; c < 3; c++) {
    const carGroup = new THREE.Group();
    const carBody = new THREE.Mesh(
      new THREE.BoxGeometry(1.2, 1.1, 4.2),
      new THREE.MeshStandardMaterial({ color: c === 0 || c === 2 ? 0xe6ecf2 : 0x00bcd4, roughness: 0.2, metalness: 0.8 })
    );
    carBody.position.y = 0.8;
    carGroup.add(carBody);

    // Glowing passenger windows strip
    const winStripL = new THREE.Mesh(
      new THREE.BoxGeometry(0.04, 0.4, 3.4),
      new THREE.MeshBasicMaterial({ color: 0x00e5ff })
    );
    winStripL.position.set(-0.61, 0.85, 0);
    const winStripR = winStripL.clone();
    winStripR.position.set(0.61, 0.85, 0);
    carGroup.add(winStripL, winStripR);

    if (c === 0) { // Front nose headlight
      const headHL = new THREE.Mesh(new THREE.SphereGeometry(0.18), new THREE.MeshBasicMaterial({ color: 0xffffff }));
      headHL.position.set(0, 0.7, 2.15);
      carGroup.add(headHL);
    }
    if (c === 2) { // Rear red tail light
      const tailHL = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.15, 0.05), new THREE.MeshBasicMaterial({ color: 0xff0044 }));
      tailHL.position.set(0, 0.7, -2.15);
      carGroup.add(tailHL);
    }

    carGroup.position.z = (1 - c) * 4.4;
    monorailTrain.add(carGroup);
    trainCars.push(carGroup);
  }
  monorailTrain.position.set(railX, railY, -120);
  monorailGroup.add(monorailTrain);
  world.add(monorailGroup);
  let trainSpeed = 0.18;
  let trainPosZ = -120;

  // ==========================================
  // 2. ILLUMINATED FERRIS WHEEL ON THE PIER
  // ==========================================
  const ferrisWheelGroup = new THREE.Group();
  const fwX = -38, fwY = 8.5, fwZ = -25;
  
  // Support legs
  const legMat = new THREE.MeshStandardMaterial({ color: 0x222a36, roughness: 0.6, metalness: 0.6 });
  const leg1 = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.25, 12), legMat);
  leg1.position.set(-1.8, 4.5, -0.8); leg1.rotation.z = -0.22;
  const leg2 = leg1.clone();
  leg2.position.set(1.8, 4.5, -0.8); leg2.rotation.z = 0.22;
  const leg3 = leg1.clone();
  leg3.position.set(-1.8, 4.5, 0.8); leg3.rotation.z = -0.22;
  const leg4 = leg1.clone();
  leg4.position.set(1.8, 4.5, 0.8); leg4.rotation.z = 0.22;
  ferrisWheelGroup.add(leg1, leg2, leg3, leg4);

  // Wheel axis & spinning rim
  const wheelSpinGroup = new THREE.Group();
  wheelSpinGroup.position.set(0, 8.5, 0);

  const ringRadius = 7.0;
  const outerRingGeom = new THREE.TorusGeometry(ringRadius, 0.12, 8, 32);
  const ringMat = new THREE.MeshBasicMaterial({ color: 0xff00aa });
  const outerRing = new THREE.Mesh(outerRingGeom, ringMat);
  wheelSpinGroup.add(outerRing);

  const innerRingGeom = new THREE.TorusGeometry(ringRadius * 0.65, 0.08, 8, 32);
  const innerRing = new THREE.Mesh(innerRingGeom, new THREE.MeshStandardMaterial({ color: 0x334455 }));
  wheelSpinGroup.add(innerRing);
  ...
  for (let i = 0; i < numGondolas; i++) {
    ...
    const spoke = new THREE.Mesh(
      new THREE.CylinderGeometry(0.04, 0.04, ringRadius * 2),
      new THREE.MeshStandardMaterial({ color: 0x445566 })
    );
    spoke.rotation.z = angle;
    wheelSpinGroup.add(spoke);

    // Neon spoke light mesh
    const neonLight = new THREE.Mesh(
      new THREE.CylinderGeometry(0.05, 0.05, ringRadius * 0.9),
      new THREE.MeshBasicMaterial({ color: 0x00ffff })
    );
    neonLight.rotation.z = angle;
    neonLight.position.set(Math.sin(angle) * ringRadius * 0.45, Math.cos(angle) * ringRadius * 0.45, 0.06);
    wheelSpinGroup.add(neonLight);
    fwSpokeLights.push(neonLight);

    // Gondola cabin
    const gondolaGrp = new THREE.Group();
    const gondolaBody = new THREE.Mesh(
      new THREE.BoxGeometry(0.9, 1.0, 0.9),
      new THREE.MeshStandardMaterial({ color: [0xff0055, 0x00ffcc, 0xffcc00, 0xaa00ff][i % 4], roughness: 0.3 })
    );
    gondolaBody.position.y = -0.5;
    const gondolaWin = new THREE.Mesh(
      new THREE.BoxGeometry(0.7, 0.4, 0.92),
      new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.7 })
    );
    gondolaWin.position.y = -0.4;
    gondolaGrp.add(gondolaBody, gondolaWin);

    gondolaGrp.position.set(Math.sin(angle) * ringRadius, Math.cos(angle) * ringRadius, 0);
    wheelSpinGroup.add(gondolaGrp);
    gondolas.push({ group: gondolaGrp, angle });
  }

  ferrisWheelGroup.add(wheelSpinGroup);
  ferrisWheelGroup.position.set(fwX, 0, fwZ);
  world.add(ferrisWheelGroup);

  buildings.push({
    group: ferrisWheelGroup,
    home: ferrisWheelGroup.position.clone(),
    width: 14,
    height: 16,
    cracks: null,
    crackMaterial: null,
  ...
  // ==========================================
  // 3. CONSTRUCTION SKYSCRAPER & TOWER CRANE
  // ==========================================
  const constSite = new THREE.Group();
  const csX = 20, csZ = -45, csH = 16;
  
  for (let f = 0; f < 5; f++) {
    const slab = new THREE.Mesh(
      new THREE.BoxGeometry(7.5, 0.3, 7.5),
      new THREE.MeshStandardMaterial({ color: 0x5a636e, roughness: 0.9 })
    );
    slab.position.y = f * 3.2 + 1.6;
    constSite.add(slab);

    for (let px of [-3.2, 3.2]) {
      for (let pz of [-3.2, 3.2]) {
        const col = new THREE.Mesh(
          new THREE.BoxGeometry(0.3, 3.2, 0.3),
          new THREE.MeshStandardMaterial({ color: 0xe67e22, roughness: 0.5, metalness: 0.7 })
        );
        col.position.set(px, f * 3.2 + 1.6, pz);
        constSite.add(col);
      }
    }
  }
  ...
  craneGroup.position.set(0, csH, 0);

  const craneMat = new THREE.MeshStandardMaterial({ color: 0xf1c40f, roughness: 0.4, metalness: 0.6 });
  const towerMast = new THREE.Mesh(new THREE.BoxGeometry(0.8, 12, 0.8), craneMat);
  towerMast.position.y = 6;
  craneGroup.add(towerMast);

  const craneJibGroup = new THREE.Group();
  craneJibGroup.position.y = 11.5;

  const jibFront = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.4, 16), craneMat);
  jibFront.position.z = 6;
  const jibBack = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.6, 5), craneMat);
  jibBack.position.z = -2.5;
  const counterWeight = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.0, 1.2), new THREE.MeshStandardMaterial({ color: 0x34495e }));
  counterWeight.position.z = -4.5;

  const beaconLight = new THREE.Mesh(new THREE.SphereGeometry(0.15), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
  beaconLight.position.y = 0.5;

  craneJibGroup.add(jibFront, jibBack, counterWeight, beaconLight);

  const cableGeom = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0, 10), new THREE.Vector3(0, -6, 10)]);
  const craneCable = new THREE.Line(cableGeom, new THREE.LineBasicMaterial({ color: 0x111111 }));
  const cargoCrate = new THREE.Mesh(new THREE.BoxGeometry(1.4, 1.2, 2.2), new THREE.MeshStandardMaterial({ color: 0x27ae60, roughness: 0.6 }));
  cargoCrate.position.set(0, -6.6, 10);
  craneJibGroup.add(craneCable, cargoCrate);

  craneGroup.add(craneJibGroup);
  constSite.add(craneGroup);

  constSite.position.set(csX, 0, csZ);
  world.add(constSite);

  buildings.push({
    group: constSite,
    home: constSite.position.clone(),
    width: 8,
    height: csH + 12,
    cracks: null,
    crackMaterial: null,
  ...
  for (let i = 0; i < rainCount; i++) {
    ...
    const dropLen = 1.2 + Math.random() * 1.5;
    rainPositions[i * 6] = rx;
    rainPositions[i * 6 + 1] = ry;
    rainPositions[i * 6 + 2] = rz;
    rainPositions[i * 6 + 3] = rx - 0.2;
    rainPositions[i * 6 + 4] = ry - dropLen;
    rainPositions[i * 6 + 5] = rz;
  }
  const rainGeom = new THREE.BufferGeometry();
  rainGeom.setAttribute('position', new THREE.BufferAttribute(rainPositions, 3));
  const rainMat = new THREE.LineBasicMaterial({ color: 0x88bbdd, transparent: true, opacity: 0.45 });
  ...
  if (rainButton) {
    rainButton.addEventListener('click', () => {
      ...
      rainLines.visible = rainEnabled;
      rainButton.textContent = rainEnabled ? '🌧️ Desactivar lluvia' : '🌧️ Activar lluvia';
    });
  }

  // ==========================================
  // 5. SWIRLING LEAVES & STORM DEBRIS
  // ==========================================
  const leavesList = [];
  const leafColors = [0xe74c3c, 0xf39c12, 0x27ae60, 0xd35400];
  for (let i = 0; i < 160; i++) {
    const leaf = new THREE.Mesh(
      new THREE.PlaneGeometry(0.18, 0.28),
      new THREE.MeshBasicMaterial({ color: leafColors[i % leafColors.length], side: THREE.DoubleSide })
    );
    const lx = (Math.random() - 0.5) * 200;
    const lz = (Math.random() - 0.5) * 200;
    leaf.position.set(lx, 0.2 + Math.random() * 5, lz);
    world.add(leaf);
    leavesList.push({
      mesh: leaf,
      pos: leaf.position,
      rotSpeed: new THREE.Vector3((Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4)
    });
  }
  ...
  function frame() {
    ...
    // Day / Night cycle logic & smooth lighting updates
    let dayTimeCycle = (t * 0.025) % 1.0;
    if (dayNightMode === 'day') dayTimeCycle = 0.25;
    else if (dayNightMode === 'night') dayTimeCycle = 0.75;

    const sunAngle = dayTimeCycle * Math.PI * 2 - Math.PI / 2;
    sun.position.x = Math.cos(sunAngle) * 160;
    sun.position.y = Math.sin(sunAngle) * 160;
    sun.position.z = Math.sin(sunAngle * 0.5) * 50;

    const sunY = Math.sin(sunAngle);
    const nightFactor = THREE.MathUtils.smoothstep(-sunY, -0.3, 0.4);

    if (sunY > 0.2) {
      targetTopColor.setHex(0x2874a6);
      targetHorizColor.setHex(0x85c1e9);
      targetSunColor.setHex(0xfff8e7);
      targetHemiSkyColor.setHex(0x85c1e9);
      targetHemiGroundColor.setHex(0x2e4053);
      targetFogColor.setHex(0x76d7ea);
    } else if (sunY > -0.2) {
      const tRatio = (sunY + 0.2) / 0.4;
      targetTopColor.setHex(0x3b1e54).lerp(new THREE.Color(0x2874a6), tRatio);
      targetHorizColor.setHex(0xe74c3c).lerp(new THREE.Color(0x85c1e9), tRatio);
      targetSunColor.setHex(0xff8833).lerp(new THREE.Color(0xfff8e7), tRatio);
      targetHemiSkyColor.setHex(0xffaa55).lerp(new THREE.Color(0x85c1e9), tRatio);
      targetHemiGroundColor.setHex(0x221122).lerp(new THREE.Color(0x2e4053), tRatio);
      targetFogColor.setHex(0x6c3483).lerp(new THREE.Color(0x76d7ea), tRatio);
    } else {
      targetTopColor.setHex(0x190847);
      targetHorizColor.setHex(0xb81878);
      targetSunColor.setHex(0x4466aa);
      targetHemiSkyColor.setHex(0xa059c2);
      targetHemiGroundColor.setHex(0x140c21);
      targetFogColor.setHex(0x260742);
    }

    skyMaterial.uniforms.top.value.lerp(targetTopColor, 0.05);
    skyMaterial.uniforms.horizon.value.lerp(targetHorizColor, 0.05);
    sun.color.lerp(targetSunColor, 0.05);
    sun.intensity = THREE.MathUtils.lerp(sun.intensity, sunY > -0.1 ? 1.6 : 0.5, 0.05);
    hemiLight.color.lerp(targetHemiSkyColor, 0.05);
    hemiLight.groundColor.lerp(targetHemiGroundColor, 0.05);
    hemiLight.intensity = THREE.MathUtils.lerp(hemiLight.intensity, sunY > -0.1 ? 1.8 : 1.2, 0.05);
    scene.fog.color.lerp(targetFogColor, 0.05);
    scene.fog.density = THREE.MathUtils.lerp(scene.fog.density, sunY > -0.1 ? 0.0035 : 0.006, 0.05);

    windowMaterial.opacity = 0.12 + nightFactor * 0.68;
    cars.forEach(item => {
      if (item.car && item.car.children) {
        item.car.children.forEach(c => {
          if (c.geometry && c.geometry.type === 'PlaneGeometry' && c.material.blending === THREE.AdditiveBlending) {
            c.material.opacity = nightFactor * 0.22;
          }
        });
      }
    });
    ...
    if (audio.enabled && audio.context) {
      ...
      const distToTornado = camera.position.distanceTo(tornado.position);
      const tornadoProximity = Math.max(0, 1 - distToTornado / 120);

      // Howling wind modulation
      const howlFreq = 300 + Math.sin(t * 1.8) * 220 + tornadoProximity * 350;
      audio.windFilter.frequency.setTargetAtTime(howlFreq, now, 0.2);
      audio.windGain.gain.setTargetAtTime(0.04 + tornadoProximity * 0.14, now, 0.2);

      // Sub-bass roar modulation
      const subFreq = 60 + tornadoProximity * 90;
      audio.subRumbleFilter.frequency.setTargetAtTime(subFreq, now, 0.2);
      audio.subRumbleGain.gain.setTargetAtTime(0.05 + tornadoProximity * 0.28, now, 0.2);

      // Rain sound level
      const rainVol = rainEnabled ? 0.035 + tornadoProximity * 0.025 : 0.0;
      audio.rainGain.gain.setTargetAtTime(rainVol, now, 0.3);

      // Tsunami ocean surge sound level
      const tsunamiVol = tsunami ? Math.min(0.25, (tsunami.magnitude / 10) * 0.25) : 0.0;
      audio.tsunamiGain.gain.setTargetAtTime(tsunamiVol, now, 0.4);

      // Earthquake rumble
      const quakeVol = earthquake ? 0.01 + earthquake.magnitude * 0.018 : 0.0;
      audio.quakeGain.gain.setTargetAtTime(quakeVol, now, 0.1);
    }
    ...
    // Flap wings and fly birds
    birds.forEach(bird => {
      bird.phase += dt * 3.5;
      bird.wingL.rotation.z = Math.sin(bird.phase * 3) * 0.5;
      bird.wingR.rotation.z = -Math.sin(bird.phase * 3) * 0.5;
      const angle = bird.phase * 0.15;
      bird.group.position.x = bird.centerX + Math.sin(angle) * bird.radius;
      bird.group.position.z = bird.centerZ + Math.cos(angle) * bird.radius;
    ...
    kites.forEach(kite => {
      kite.phase += dt * 2;
      kite.group.position.x = -29.8 + Math.sin(kite.phase) * 0.8;
      kite.group.position.y = kite.homeY + Math.cos(kite.phase * 1.5) * 0.4;
      kite.group.rotation.z = Math.sin(kite.phase * 2) * 0.15;
    });

    // Hover drones
    drones.forEach(drone => {
      drone.phase += dt * 3.5;
      drone.group.position.x = drone.home.x + Math.sin(drone.phase) * 1.2;
      drone.group.position.y = drone.home.y + Math.sin(drone.phase * 2) * 0.3;
      drone.group.position.z = drone.home.z + Math.cos(drone.phase * 0.8) * 1.2;
      const blink = Math.floor(t * 5) % 2 === 0;
      drone.rLight.visible = blink;
      drone.gLight.visible = !blink;
    });

    // Rotate lighthouse beam
    if (lighthouseSpot && !lighthouse.destroyed) {
      const angle = t * 1.1;
      lighthouseSpot.target.position.set(
        lighthouse.position.x + Math.sin(angle) * 15,
        1.5,
        lighthouse.position.z + Math.cos(angle) * 15
      );
      beamCone.lookAt(lighthouseSpot.target.position);
      beamCone.rotation.x += Math.PI / 2;
    } else if (lighthouse.destroyed) {
      if (lighthouseSpot) {
        lighthouse.remove(lighthouseSpot);
        lighthouse.remove(beamCone);
        lighthouseSpot = null;
      }
    }
    ...
    buoys.forEach(buoy => {
      buoy.phase += dt * 3.5;
      buoy.group.position.y = Math.sin(t * 1.4 + buoy.group.position.x) * 0.15;
      buoy.group.rotation.z = Math.sin(t * 1.4 + buoy.group.position.x) * 0.08;
      const flash = Math.floor(t * 4) % 2 === 0;
      buoy.light.material.color.setHex(flash ? buoy.color : 0x110000);
    });

    // Animate dolphins jumping in sea
    dolphins.forEach(dolphin => {
      dolphin.progress += dt * dolphin.speed;
      if (dolphin.progress > Math.PI) {
        dolphin.progress = 0;
        dolphin.x = -40 + (Math.random() - 0.5) * 15;
        dolphin.z = -120 + Math.random() * 240;
      }
      const jumpHeight = Math.sin(dolphin.progress) * 1.5;
      dolphin.mesh.position.set(dolphin.x + Math.cos(dolphin.progress) * 1.5, jumpHeight - 0.15, dolphin.z);
      dolphin.mesh.rotation.x = Math.cos(dolphin.progress) * -0.4;
    });
    ...
    trainPosZ += trainSpeed;
    if (trainPosZ > 135) trainPosZ = -135;
    monorailTrain.position.z = trainPosZ;

    // Animate Ferris Wheel & Gondolas
    if (!ferrisWheelGroup.destroyed) {
      wheelSpinGroup.rotation.z += dt * 0.35;
      gondolas.forEach(g => {
        g.group.rotation.z = -wheelSpinGroup.rotation.z;
      });
      fwSpokeLights.forEach((light, idx) => {
        const hue = (t * 0.3 + idx * 0.15) % 1;
        light.material.color.setHSL(hue, 1.0, 0.5);
      });
      outerRing.material.color.setHSL((t * 0.25) % 1, 1.0, 0.5);
    }

    // Animate Construction Crane Jib & Swaying Cargo Crate
    if (!constSite.destroyed) {
      craneJibGroup.rotation.y = Math.sin(t * 0.4) * 0.85;
      cargoCrate.rotation.z = Math.sin(t * 1.8) * 0.14;
      cargoCrate.rotation.x = Math.cos(t * 1.5) * 0.10;
      beaconLight.material.color.setHex(Math.floor(t * 6) % 2 === 0 ? 0xff0000 : 0x220000);
    }

    // Animate Rainfall System
    if (rainEnabled) {
    ...
    // Animate Swirling Storm Leaves
    leavesList.forEach(l => {
      l.mesh.rotation.x += l.rotSpeed.x * dt;
      l.mesh.rotation.y += l.rotSpeed.y * dt;
      l.mesh.rotation.z += l.rotSpeed.z * dt;

      const distToTornado = Math.hypot(l.pos.x - tornado.position.x, l.pos.z - tornado.position.z);
      if (distToTornado < 18) {
        const angle = Math.atan2(l.pos.z - tornado.position.z, l.pos.x - tornado.position.x) + dt * 4.5;
        const rad = Math.max(0.5, distToTornado - dt * 2.5);
        l.pos.x = tornado.position.x + Math.cos(angle) * rad;
        l.pos.z = tornado.position.z + Math.sin(angle) * rad;
        l.pos.y = Math.min(22, l.pos.y + dt * 8);
        if (l.pos.y > 20) {
          l.pos.set((Math.random() - 0.5) * 180, 0.2 + Math.random() * 2, (Math.random() - 0.5) * 180);
        }
      } else {
        l.pos.x += Math.sin(t * 1.5 + l.pos.z) * dt * 1.5;
        l.pos.z += Math.cos(t * 1.2 + l.pos.x) * dt * 1.5;
        if (l.pos.y > 0.2) l.pos.y -= dt * 0.8;
      }
    });
